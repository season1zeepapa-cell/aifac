...
> 실행일: 2026-03-11
> 테스트 파일: `tests/search-advanced.spec.js`
> 대상 URL: https://docstore-eight.vercel.app
> 브라우저: Chromium (headless)
> 전체 소요시간: 약 1분 6초
...
#### 1. 검색 UI 기본 요소 (3/3 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 1 | 검색 입력창, 버튼, 모드 선택이 표시된다 | 1.3s | 입력창, 검색 버튼, 통합/텍스트/의미 모드 버튼, 필터 버튼 |
| 2 | 검색 모드 전환이 동작한다 | 1.5s | 기본 통합→텍스트→의미 모드 전환 시 활성 클래스 변경 |
| 3 | 빈 검색어로 검색 시 버튼이 비활성화된다 | 1.1s | disabled 속성 확인 |

#### 2. 자동완성 드롭다운 (5/5 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 4 | 2글자 이상 입력 시 자동완성 드롭다운이 나타난다 | 2.2s | 디바운스(250ms) 후 suggest API 호출 |
| 5 | 자동완성 API가 suggest 파라미터와 함께 호출된다 | 1.8s | API 200 응답 + suggestions 배열 반환 |
| 6 | 자동완성 결과에 문서 타입과 본문 타입이 구분되어 표시된다 | 1.9s | "문서"/"본문" 뱃지 렌더링 |
| 7 | 키보드 화살표로 자동완성 항목을 이동할 수 있다 | 1.9s | ArrowDown → 활성 항목, Escape → 닫기 |
| 8 | 자동완성 하단에 키보드 단축키 안내가 표시된다 | 1.7s | "이동", "선택", "닫기" 텍스트 |

#### 3. 검색 실행 및 결과 (5/5 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 9 | 통합 검색(hybrid)이 결과를 반환한다 | 2.5s | type=hybrid, results 배열 |
| 10 | 텍스트 검색(FTS)이 결과를 반환한다 | 2.6s | type=fts 또는 text |
| 11 | 의미 검색(vector)이 결과를 반환한다 | 2.9s | type=vector |
| 12 | 검색 결과 0건이면 빈 상태 메시지가 표시된다 | 2.4s | "검색 결과가 없습니다" 표시 |
| 13 | Enter 키로 검색이 실행된다 | 2.1s | Enter → API 호출 200 |

#### 4. 검색 결과 하이라이팅 (3/3 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 14 | FTS 검색 결과에 mark 하이라이팅이 표시된다 | 2.0s | `.fts-headline mark` 요소 존재 |
| 15 | 통합 검색 결과에도 하이라이팅이 적용된다 | 3.8s | `.fts-headline mark` 또는 `.search-mark` |
| 16 | 하이라이트된 mark 요소에 스타일이 적용되어 있다 | 2.5s | font-weight ≥ 500 (computed style) |

#### 5. 검색 결과 카드 상호작용 (4/4 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 17 | 검색 결과 카드 클릭 시 문서 상세 모달이 열린다 | 3.3s | 카드 클릭 → 모달 이벤트 |
| 18 | 통합 검색 결과에 매칭 방식 뱃지가 표시된다 | 3.9s | 양쪽 매칭/의미 매칭/키워드 매칭 뱃지 |
| 19 | 통합 검색 결과에 RRF 품질 바가 표시된다 | 3.3s | "RRF" 텍스트 표시 |
| 20 | 더보기 버튼으로 추가 결과를 로드한다 | 2.4s | 5건 초과 시 더보기 버튼 |

#### 6. 검색 필터 (2/2 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 21 | 필터 버튼 클릭 시 필터 패널이 열린다 | 2.0s | 필터 클릭 → "문서 범위" 표시 |
| 22 | 필터 패널에 문서 범위 멀티셀렉트가 있다 | 2.1s | 문서 범위 선택 영역 |

#### 7. 검색 API 응답 구조 검증 (4/4 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 23 | hybrid 검색 응답에 필수 필드가 포함된다 | 2.2s | type, query, count, results, chunkText, rrfScore, documentTitle, category |
| 24 | FTS 검색 응답에 headline 필드가 포함된다 | 2.6s | headline(mark 태그 포함), ftsScore, rawText |
| 25 | hybrid 검색 응답에 headline이 포함된다 (FTS 매칭 시) | 3.0s | ftsRank가 있는 결과에 headline 포함 |
| 26 | vector 검색 응답에 similarity가 포함된다 | 2.7s | similarity: 0~1 범위 float |

#### 8. 자동완성 API 응답 구조 (1/1 통과)

| # | 테스트명 | 시간 | 검증 항목 |
|---|---------|------|----------|
| 27 | suggest API가 문서/섹션 타입을 구분하여 반환한다 | 1.8s | suggestions[].type: "document"/"section", section에는 doc_title 포함 |

### 검색 아키텍처 테스트 커버리지

```
검색 파이프라인
├── [검증됨] UI 렌더링: 입력창 + 버튼 + 모드 전환
├── [검증됨] 자동완성: API 호출 → 드롭다운 → 키보드 내비게이션
├── [검증됨] 검색 실행: 3가지 모드(hybrid/FTS/vector) + Enter 키
├── [검증됨] 결과 표시: 하이라이팅(서버/클라이언트) + 뱃지 + 점수 바
├── [검증됨] 상호작용: 카드 클릭 → 모달, 더보기 페이징
├── [검증됨] 필터: 패널 열기 + 문서 범위 선택
└── [검증됨] API 응답: 필수 필드 + headline + similarity 구조
```
...
| 이슈 | 원인 | 해결 |
|------|------|------|
| `fontWeight` NaN 에러 | `getComputedStyle().fontWeight`가 "bold" 문자열 반환 시 `parseInt` NaN | "bold"→700, "normal"→400 매핑 추가 |

### 테스트 실행 방법

```bash
cd workspace/docstore

# 검색 전체 테스트
npx playwright test tests/search-advanced.spec.js --project=chromium --no-deps

# 특정 카테고리만
npx playwright test tests/search-advanced.spec.js -g "자동완성" --no-deps
npx playwright test tests/search-advanced.spec.js -g "하이라이팅" --no-deps
