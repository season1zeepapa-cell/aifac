...
- [ ] 전일 대비 증감률 계산 로직 추가 (API 측에서 전일 데이터 함께 반환)
- [ ] 총 토큰(입력+출력) 요약 카드 추가
...
- [ ] `editingLimit` 상태 추가 (프로바이더별 편집 모드)
- [ ] 한도 클릭 → 인라인 input 전환
- [ ] Enter 또는 저장 버튼으로 반영

#### 2-B. 키 설정 가이드

- [ ] 미설정 키에 "설정 방법" 접기/펼치기 안내 추가
- [ ] 각 프로바이더별 환경변수명 + 발급 URL 링크
- [ ] 예: `OPENAI_API_KEY` → "OpenAI Platform에서 발급"

#### 2-C. 일괄 테스트

- [ ] "전체 키 테스트" 버튼 추가 (설정된 키 순차 테스트)
- [ ] 진행 상황 표시 (예: "2/4 테스트 중...")
- [ ] 결과 요약 (성공 3개, 실패 1개)
...
- [ ] CSV 다운로드 버튼 (모델별 사용량 데이터)
- [ ] 기간 범위 포함된 파일명 자동 생성
- [ ] 예: `docstore-usage-2026-03-04-to-2026-03-10.csv`
...
- [ ] "전체 테스트" 버튼 (활성화+사용 가능 엔진 대상)
- [ ] 순차 실행 + 진행률 표시 (1/4, 2/4...)
- [ ] 결과 요약 카드 (성공 N개, 실패 N개)

#### 4-B. OCR 사용 통계

- [ ] 최근 7일 OCR 엔진별 호출 수 / 성공률 표시
- [ ] `api_usage` 테이블에서 endpoint='ocr'인 데이터 집계
...
| 순위 | 항목 | 탭 | 난이도 | 효과 |
|------|------|-----|--------|------|
| 1 | 일별 추이 시각화 | 대시보드 | 중 | `dailyTrend` 데이터 활용, 유용도 높음 |
| 2 | 합계 행 추가 | 사용량 | 낮 | 프론트만 수정, 즉시 적용 가능 |
| 3 | 한도 설정 인라인 UI | API 키 | 낮 | prompt() 제거, UX 대폭 향상 |
| 4 | 일괄 테스트 (키+OCR) | API 키, OCR | 낮 | 편의성 향상 |
| 5 | 요약 카드 보강 | 대시보드 | 중 | API 수정 필요 (전일 데이터) |
| 6 | OCR 사용 통계 | OCR | 중 | API 쿼리 추가 필요 |
| 7 | 사용량 내보내기 | 사용량 | 낮 | CSV 다운로드, 프론트만 |
| 8 | 에러 로그 필터 | 사용량 | 낮 | 프론트 필터링 |
| 9 | 키 설정 가이드 | API 키 | 낮 | 정적 텍스트 추가 |
| 10 | 드래그 앤 드롭 | OCR | 높 | 터치 지원 복잡도 |

### 예상 작업 단위

```
Phase 1 (프론트만 수정, 즉시 적용)
  ├── #2 합계 행 추가 (사용량)
