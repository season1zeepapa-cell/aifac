...
module.exports = async (req, res) => {
  ...
  try {
    if (action === 'search') {
      ...
      let importedMap = {};
      if (lawIds.length > 0) {
        const { clause: lawOrgC, params: lawOrgP } = orgFilter(orgId, 'd', 2);
        const lawOrgWhere = lawOrgC ? ` AND ${lawOrgC}` : '';
        const imported = await dbQuery(
          `SELECT metadata->>'lawId' AS law_id, id, title, upload_date
           FROM documents d
           WHERE metadata->>'lawId' = ANY($1) AND deleted_at IS NULL${lawOrgWhere}`,
          [lawIds, ...lawOrgP]
        );
